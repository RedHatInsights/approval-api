RSpec.describe AddRemoteTags, :type => :request do
  ...
  context 'catalog inventory' do
    ...
    context 'credentials' do
      ...
      let(:url)         { "http://localhost/api/catalog-inventory/v1.0/credentials/#{object_id}/tag" }
      it_behaves_like "#test_all"
    end

    context 'ServiceInventory' do
      let(:object_type) { 'ServiceInventory' }
      let(:url)         { "http://localhost/api/catalog-inventory/v1.0/service_inventories/#{object_id}/tag" }
      it_behaves_like "#test_all"
    end
  end
