  ...
  describe "Routing" do
    ...
    context "customizable route prefixes" do
      ...
      it "with a random prefix" do
        ...
        expect(api_v0x1_requests_url(:only_path => true)).to eq("/#{URI.encode(ENV["PATH_PREFIX"])}/#{URI.encode(ENV["APP_NAME"])}/v0.1/requests")
      end
      ...
      it "with extra slashes" do
        ...
        expect(api_v0x1_requests_url(:only_path => true)).to eq("/example/path/prefix/appname/v0.1/requests")
      end
      ...
      it "doesn't use the APP_NAME when PATH_PREFIX is empty" do
        ...
        expect(ENV["APP_NAME"]).not_to be_nil
        expect(api_v0x1_requests_url(:only_path => true)).to eq("/api/v0.1/requests")
      end
    end
