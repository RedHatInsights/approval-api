RSpec.describe Approver, :type => :model do
  ...
  let!(:adam) { create(:approver, :group_ids => [groups_a.first.id, groups_b.first.id, groups_b.last.id]) }
  let!(:fred) { create(:approver, :group_ids => [groups_a.first.id, groups_b.second.id]) }
  let!(:john) { create(:approver, :group_ids => [groups_a.last.id, groups_b.last.id]) }

  it { should have_many(:approvergroups) }
  it { should have_many(:groups).through(:approvergroups) }
  it { should validate_presence_of(:email) }

  context 'all associations' do
    it 'when list groups by approvers' do
      expect(adam.groups.count).to eq(3)
      expect(fred.groups.count).to eq(2)
      expect(john.groups.count).to eq(2)
    end

    it 'when list stages by approvers' do
      expect(adam.stages.count).to eq(3)
      expect(fred.stages.count).to eq(2)
      expect(john.stages.count).to eq(2)
    end

    it 'when list requests by approvers' do
